...
import com.finwin.doorstep.rightviewfinance.home.transactions.cash_deposit.dialog.ConfirmationViewmodel
import com.finwin.doorstep.rightviewfinance.home.transactions.search_account.SearchActivity
import com.finwin.doorstep.rightviewfinance.print_reciept.ReceiptActivity
import com.finwin.doorstep.rightviewfinance.utils.Constants

import com.finwin.doorstep.rightviewfinance.R
import com.finwin.doorstep.rightviewfinance.databinding.FragmentCashDepositBinding
import com.finwin.doorstep.rightviewfinance.databinding.LayoutCollectionConfirmationBinding
...
class CashDepositFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding=DataBindingUtil.inflate(inflater, R.layout.fragment_cash_deposit, container, false)
        ...
        viewmodel.mAction.observe(viewLifecycleOwner, Observer {
            ...
            when(it.action)
            {
                CashDepositAction.CLICK_SEARCH->
                {
                    ...
                    startActivityForResult(intent, Constants.INTENT_SEARCH_ACCOUNT_FROM_CASH_DEPOSIT)
                }
                ...
                CashDepositAction.CASH_DEPOSIT_SUCCESS->
                {
                    ...
                    intent.putExtra(Constants.TRANSACTION_ID,it.cashDepositResponse.receipt.data.TRAN_ID)
                    intent.putExtra(Constants.OLD_BALANCE,it.cashDepositResponse.receipt.data.OLD_BALANCE)
                    intent.putExtra(Constants.DEPOSIT_AMOUNT,it.cashDepositResponse.receipt.data.DEPOSIT_AMOUNT)
                    intent.putExtra(Constants.CURRENT_BALANCE,it.cashDepositResponse.receipt.data.CURRENT_BALANCE)
                    intent.putExtra(Constants.DEPOSIT_DATE,it.cashDepositResponse.receipt.data.DEPOSIT_DATE)
                    intent.putExtra(Constants.PREVIOUS_BALANCE,it.cashDepositResponse.receipt.data.OLD_BALANCE)
                    intent.putExtra(Constants.ACCOUNT_NUMBER,it.cashDepositResponse.receipt.data.ACC_NO)
